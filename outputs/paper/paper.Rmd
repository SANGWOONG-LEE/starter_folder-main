...
```
It is bit difficult to observe that which division corresponds to line since there are many lines. Let's put that besides, what we want from this project is observing decreasing patterns so we can learn successful startegies we have done in that specific region and year. Clealry from the grpah, there seems many things that we can learn from.
